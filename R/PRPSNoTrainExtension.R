...
#' PRPS score calculation for a testing new data set based on PRPS_SLwithTraitsWeights object
#' @description This is the function to calculate PRPS (Probability ratio based classification predication score)
#'  scores for a testing data set with PRPS_SLwithTraitsWeights self learning object. The selected feature list, 
#'  these features' parameters are from the given PRPS_SLwithTraitsWeights object.
...
#'  the self learning data set as much as possible. Within PRPS_SLwithTraitsWeights and within this current PRPStesting functions, 
...
#'  However, if we know that the testing and self learning data sets are not comparable, we should set isCompToTrain as
...
#' @param PRPS_SLObj a PRPS_SLwithTraitsWeights self learning object, which is the output from function PRPS_SLwithTraitsWeights
...
PRPSNoTrainExtension = function(PRPS_SLObj, newdat, standardization=FALSE,  classProbCut = 0.8,
  ...
  #refGroup = setdiff(unique(PRPS_SLObj$PRPS_train$PRPS_class),testGroup)
