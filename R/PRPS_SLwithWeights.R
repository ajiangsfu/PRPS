...
#' which is written as PRPS_SLwithWeightsPrior function in this package. We also have another function: PRPS_SLwithWeightsEM, 
...
#' selected features and their weights. It combines ideas from PRPS_SLwithWeightsPrior and
#' PRPS_SLwithWeightsEM, which we call it as self learning algorithm, the actual steps are as following:
#' 1) assume that we have a pool for group ratio priors such as: seq(0.05, 0.95, by = 0.05), this will give us 19 ratio priors
#' 2) for each prior in 1), call PRPS_SLwithWeightsPrior to achieve PRPS scores
...
#' @param indat a input data matrix or data frame, columns for samples and rows for features
...
#' \item{PRPS_test}{a data frame of PRPS score, true classification, Empirical Bayesian probabilites for both groups, and its classification 
#' for all training samples, notice that there are two ways for classifications, one (PRPS_class) is based on probabilities, 
#' and there is UNCLASS group besdies the given two groups, alternatively, the other one (PRPS_class0) is based on PRPS scores directly
#' and 0 treated as a natural cutoff}
#' @keywords PRPS EM 
...
PRPS_SLwithWeights = function(indat, weights, standardization=FALSE, classProbCut = 0.8, PRPShighGroup = "PRPShigh", 
                    PRPSlowGroup = "PRPSlow", breaks = 50, EMmaxRuns = 50, imputeNA = FALSE, byrow = TRUE, imputeValue = c("median","mean")){
  require(mclust)
  imputeValue = imputeValue[1]
  ## imputee NA if imputeNA is true
  if(imputeNA){
    indat = imputeNAs(dataIn = indat, byrow = byrow, imputeValue = imputeValue)
  }
  ...
  rpsres = sapply(rps, FUN = function(xx){
    tmp = PRPS_SLwithWeightsPrior(newdat=indat, weights=weights, ratioPrior = xx, PRPShighGroup = PRPShighGroup, PRPSlowGroup = PRPSlowGroup)
    mcls = mclust::Mclust(tmp$PRPS_test$PRPS_score, G=2)
    return(mcls$classification)
  })
  
  rownames(rpsres) = colnames(indat)
  ...
  PRPS_score = weightedLogProbClass(newdat = indat, topTraits=names(weights), weights=weights,
  ...
  if(emcut$Means[1]> emcut$Means[2]){
    name1 = PRPShighGroup
    name2 = PRPSlowGroup
    scoreMeanSds = c(emcut$Means, emcut$SDs)
  }else{
    name1 = PRPSlowGroup
    name2 = PRPShighGroup
    scoreMeanSds = c(rev(emcut$Means), rev(emcut$SDs))
  }
